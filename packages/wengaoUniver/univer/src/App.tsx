...
function App(): ReactElement {
    ...
    const increment = () => {
        ...
        const univerAPI: FUniver = univerRef.current?.univerAPI?.current;
        if (!univerAPI) {
            throw Error("univerAPI undone");
        }
        const range = univerAPI?.getActiveWorkbook()?.getActiveSheet()?.getRange(0, 0);
        if (!range) {
            return;
        }
        const oldVal = isNaN(Number(range.getValue())) ? 0 : Number(range.getValue());
        ...
        <Fragment>
            <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                <div className="bar" style={{ display: "flex", gap: "10px" }}>
                    ...
                    <button onClick={increment} title="Increment the value of the first cell">
                        Increment
                    </button>
                    <button onClick={changeCellSize} title="Change the size of the B2 cell">
                        changeCellSize
                    </button>
                    ...
                    <button onClick={changeStyleByCommand} title="Change style by Command">
                        changeStyle2
                    </button>
                </div>
                <UniverSheet
