...
async function saveDocument(fileBlob: File, guid: string, fileName: string) {
    await new Promise((resolve, reject) => {
        // @ts-ignore
        mx.data.saveDocument(guid, fileName, { /* width: 180, height: 180 */ }, fileBlob, resolve, reject);
    })
}
...
export function WengaoFileUpload(props: WengaoFileUploadContainerProps): ReactElement {
    const datasourceFileList = useMemo<UploadFile[]>(() => {
        if (props.isMultiple && props.datasource && props.datasource.status === ValueStatus.Available && props.datasource.items) {
    ...
    useEffect(() => {
        ...
        needNew && props.onNewFile && props.onNewFile.canExecute && props.onNewFile.execute();
